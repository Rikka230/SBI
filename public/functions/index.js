...
exports.adminResolveFinalizationEscalation = onCall({
    ...
    const targetData = targetDoc.data() || {};
    if (targetData.isGod === true && caller.data.isGod !== true) {
        throw new HttpsError("permission-denied", "Seul le compte Suprême peut traiter cette alerte.");
    }

    const accountStatus = targetData.accountStatus || {};
    if (!accountStatus.finalizationEscalationAt) {
        return {
            success: true,
            message: "Aucune alerte de finalisation active pour ce compte."
        };
    }

    if (accountStatus.finalizationEscalationResolvedAt) {
        return {
            success: true,
            message: "Cette alerte a déjà été marquée comme traitée."
        };
    }
    ...
    await targetRef.update({
        "accountStatus.finalizationEscalationResolvedAt": now,
        ...
        "accountStatus.finalizationEscalationResolvedByName": caller.name,
        "accountStatus.preparationState": accountStatus.preparationState || "to_check",
        updatedAt: now
    });
    ...
    await safeWriteAccountAuditLog(db, {
        ...
        targetRole: targetData.role || "",
        source: "admin-direct-contact"
    });

    return {
        success: true,
        message: "Alerte de finalisation marquée comme traitée."
    };
});
