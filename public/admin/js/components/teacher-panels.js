...
export class TeacherLeftPanel extends HTMLElement {
  connectedCallback() {
      ...
      <aside id="left-panel" class="side-panel">
        <div class="panel-header" style="display:flex; justify-content:space-between; align-items:center; padding:0 15px; width:100%; box-sizing:border-box;">
          <div class="logo-zone" style="display:flex; align-items:center; overflow:hidden; white-space:nowrap; gap:.42rem;">
            ${brand('Prof', 'var(--accent-orange, #f97316)')}
          </div>
        ...
        <ul class="nav-menu">
          ...
          <li class="nav-item ${path.includes('/teacher/livrets.html') ? 'active' : ''}" data-sbi-href="/teacher/livrets.html" role="link" tabindex="0"><svg viewBox="0 0 24 24"><path d="M4 4a2 2 0 0 1 2-2h11a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H6a2 2 0 0 0 2 2h10a1 1 0 0 0 1-1V5h1a1 1 0 0 1 1 1v15a2 2 0 0 1-2 2H6a4 4 0 0 1-4-4V4Zm4 3h6v2H8V7Zm0 4h6v2H8v-2Z"/></svg><span class="nav-text">Livrets</span></li>
          <li class="nav-item ${path.includes('/teacher/documents.html') ? 'active' : ''}" data-sbi-href="/teacher/documents.html" role="link" tabindex="0"><svg viewBox="0 0 24 24"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7Zm2 3v8h14v-8H5Z"/></svg><span class="nav-text">Documents</span></li>
        </ul>
    ...
    `;
    this.querySelector('#logout-btn-teacher')?.addEventListener('click', signOutToLogin);
    dispatchComponentMounted('teacher-left-panel', this);
  }
}

export class TeacherTopBar extends HTMLElement {
  connectedCallback() {
    if (this.dataset.rendered === 'true') return;
    this.dataset.rendered = 'true';
    ...
    this.innerHTML = `
      <header class="top-bar" style="border-bottom:1px solid var(--border-color); background-color:rgba(255,255,255,.95); backdrop-filter:blur(10px);">
        ...
        <div class="search-bar-top" style="position:relative; flex-grow:1; max-width:450px; margin-left:2rem;">
          ...
          <input type="text" class="global-search-input" placeholder="Rechercher un cours, un élève..." style="width:100%; box-sizing:border-box; padding:.7rem 1.5rem .7rem 2.8rem; background:#f9fafb; border:1px solid var(--border-color); border-radius:20px; outline:none; font-size:.95rem; color:var(--text-main);">
    ...
    `;
    dispatchComponentMounted('teacher-top-bar', this);
  }
}

export function registerTeacherPanels() {
  defineOnce('teacher-left-panel', TeacherLeftPanel);
  defineOnce('teacher-top-bar', TeacherTopBar);
}
