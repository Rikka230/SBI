...
function getEditUrl(courseId, isAdminLike) {
    const basePath = isAdminLike ? '/admin/course-editor.html' : '/teacher/course-editor.html';
    return `${basePath}?id=${encodeURIComponent(courseId)}`;
}
...
export function renderCourseActionButtons({ courseId, courseData, currentUid, isAdminLike }) {
    ...
    if (canEditCourse({ courseData, currentUid, isAdminLike })) {
        const editUrl = getEditUrl(courseId, isAdminLike);
        buttons.push(`<button class="action-btn" style="width: auto; margin: 0; color: var(--accent-blue); background: transparent; border: 1px solid var(--border-color, #333);" onclick="window.location.assign('${editUrl}')">Éditer</button>`);
    } else {
