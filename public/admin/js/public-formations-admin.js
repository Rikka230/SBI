...
export function initPublicFormationsAdmin() {
  if (mounted && document.querySelector('[data-sbi-public-formations-admin]')) return () => {};
  if (!document.querySelector('[data-sbi-public-formations-admin]')) return () => {};
  ...
  document.querySelectorAll('#public-formation-form input, #public-formation-form textarea, #public-formation-form select').forEach((field) => {
    bind(field, 'input', updatePreview);
    bind(field, 'change', updatePreview);
  });
