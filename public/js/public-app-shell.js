/**
 * SBI 8.0P.67 - Progressive PJAX page dissolve
 *
...
const PUBLIC_SHELL_VERSION = '8.0P.67';
...
async function runPageInitializers(pageId) {
  ...
  if (pageId === 'calculator') {
    try {
      const calculatorModule = await import('/js/sbi-aide-calculator.js?v=8.0P.67');
  ...
  if (['formations', 'parcours', 'apropos', 'ressources', 'contact'].includes(pageId)) {
    try {
      const publicPagesModule = await import('/js/sbi-public-pages.js?v=8.0P.67');
