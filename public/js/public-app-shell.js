...
const PUBLIC_SHELL_VERSION = '8.0P.98';
...
async function runPageInitializers(pageId) {
  ...
  if (['home', 'formations', 'ressources'].includes(pageId)) {
    try {
      const mediaModule = await import('/js/site-index-public.js?v=8.0P.98');
  ...
  if (['home', 'formations', 'parcours', 'apropos', 'ressources', 'contact'].includes(pageId)) {
    try {
      const publicPagesModule = await import('/js/sbi-public-pages.js?v=8.0P.98');
