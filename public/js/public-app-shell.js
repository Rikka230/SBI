...
const PUBLIC_SHELL_VERSION = '8.0P.88';
...
async function runPageInitializers(pageId) {
  ...
  if (pageId === 'home') {
    try {
      const mediaModule = await import('/js/site-index-public.js?v=8.0P.88');
  ...
  if (['home', 'formations', 'parcours', 'apropos', 'ressources', 'contact'].includes(pageId)) {
    try {
      const publicPagesModule = await import('/js/sbi-public-pages.js?v=8.0P.88');
