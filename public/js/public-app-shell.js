/**
 * SBI 8.0P.11 - Public media recovery and founder HD pass
 *
...
const PUBLIC_SHELL_VERSION = '8.0P.11';
...
async function runPageInitializers(pageId) {
  ...
  if (pageId === 'home') {
    try {
      const mediaModule = await import('/js/site-index-public.js?v=8.0P.11');
  ...
  if (pageId === 'calculator') {
    try {
      const calculatorModule = await import('/js/sbi-aide-calculator.js?v=8.0P.11');
  ...
  if (['formations', 'parcours', 'apropos', 'ressources', 'contact'].includes(pageId)) {
    try {
      const publicPagesModule = await import('/js/sbi-public-pages.js?v=8.0P.11');
