...
import {
  ...
  setLeftNavActive,
  loadScriptOnce
} from './admin-page-loader.js?v=8.0P.167.243';
...
async function mountLiveSchedulerV2Route({ url, role = 'teacher' }) {
  ...
  const module = await import('/js/live/live-scheduler-v2-page.js?v=8.0P.167.243');
...
async function mountStudentLives({ url }) {
  ...
  try {
    const module = await import('/student/js/student-lives.js?v=8.0P.167.243');
...
async function mountStudentLiveReplay({ url }) {
  ...
  try {
    const module = await import('/student/js/live-replay.js?v=8.0P.167.243');
