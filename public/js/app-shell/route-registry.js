...
import {
  ...
  setLeftNavActive,
  loadScriptOnce
} from './admin-page-loader.js?v=8.0P.167.247';
...
function isAdminLives(url) {
  return normalizePath(url.pathname).toLowerCase() === '/admin/admin-lives-v1.html';
}

function isAdminLivesV2(url) {
  const path = normalizePath(url.pathname).toLowerCase();
  return path === '/admin/admin-lives.html' || path === '/admin/admin-lives-v2.html' || path === '/admin/formations-live.html';
}
...
function isAdminFormationDocuments(url) {
  return normalizePath(url.pathname).toLowerCase() === '/admin/formation-documents.html';
}

function isTeacherDocuments(url) {
  return normalizePath(url.pathname).toLowerCase() === '/teacher/documents.html';
}

function isStudentDocuments(url) {
  return normalizePath(url.pathname).toLowerCase() === '/student/documents.html';
}

function isAdminBooklets(url) {
  return normalizePath(url.pathname).toLowerCase() === '/admin/apprenticeship-booklets.html';
}

function isStudentBooklet(url) {
  return normalizePath(url.pathname).toLowerCase() === '/student/livret.html';
}

function isTeacherBooklets(url) {
  return normalizePath(url.pathname).toLowerCase() === '/teacher/livrets.html';
}
...
function isTeacherLives(url) {
  return normalizePath(url.pathname).toLowerCase() === '/teacher/lives-v1.html';
}

function isTeacherLivesV2(url) {
  const path = normalizePath(url.pathname).toLowerCase();
  return path === '/teacher/lives.html' || path === '/teacher/lives-v2.html';
}
...
function isAdminShellContext() {
  ...
  return path === '/admin/index.html'
    ...
    || path === '/admin/admin-lives-v2.html'
...
function isTutorDocuments(url) {
  return normalizePath(url.pathname).toLowerCase() === '/tutor/documents.html';
}
...
async function mountAdminCourses({ url }) {
  ...
  try {
    const module = await import('/admin/js/admin-courses.js?v=8.0P.167.358');
...
async function mountAdminLateStudents({ url }) {
  ...
  try {
    const module = await import('/admin/js/admin-late-students.js?v=8.0P.167.358');
...
async function mountAdminAssignments({ url }) {
  ...
  try {
    const module = await import('/admin/js/admin-assignments.js?v=8.0P.167.358');
...
async function mountTeacherAssignments({ url }) {
  ...
  try {
    const module = await import('/teacher/js/teacher-assignments.js?v=8.0P.167.358');
...
async function mountStudentAssignments({ url }) {
  ...
  try {
    const module = await import('/student/js/student-assignments.js?v=8.0P.167.358');
...
async function mountAdminFormationDocuments({ url }) {
  cleanupCourseEditorV2Artifacts();
  maybeCacheAdminIndexMain('leave-for-admin-formation-documents');
  ...
  applyBodyRouteClassesFromDocument(doc, ['sbi-admin-surface']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Documents de formation - SBI Admin');
  setLeftNavActive('nav-formations');
  updateUrlContext(url);

  window.__SBI_APP_SHELL_MOUNTING_ADMIN_FORMATION_DOCS = true;
  try {
    const module = await import('/admin/js/admin-formation-documents.js?v=8.0P.167.304');
    ...
    window.__SBI_APP_SHELL_MOUNTING_ADMIN_FORMATION_DOCS = false;
  }

  return { viewKey: 'admin:formation-documents' };
}

async function mountTeacherDocuments({ url }) {
  cleanupCourseEditorV2Artifacts();
  ...
  applyBodyRouteClassesFromDocument(doc, ['sbi-teacher-surface', 'no-right-panel']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Documents de formation - SBI Prof');
  setLeftNavActive('/teacher/documents.html');
  updateUrlContext(url);

  window.__SBI_APP_SHELL_MOUNTING_TEACHER_DOCUMENTS = true;
  try {
    const module = await import('/teacher/js/teacher-documents.js?v=8.0P.167.304');
    const cleanup = module.mountTeacherDocuments?.();
    if (typeof cleanup === 'function') registerCleanup(cleanup, 'teacher-documents');
  } finally {
    window.__SBI_APP_SHELL_MOUNTING_TEACHER_DOCUMENTS = false;
  }

  return { viewKey: 'teacher:documents' };
}

async function mountStudentDocuments({ url }) {
  cleanupCourseEditorV2Artifacts();
  ...
  applyBodyRouteClassesFromDocument(doc, ['no-right-panel']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Mes documents - SBI Student');
  setLeftNavActive('/student/documents.html');
  updateUrlContext(url);

  window.__SBI_APP_SHELL_MOUNTING_STUDENT_DOCUMENTS = true;
  try {
    const module = await import('/student/js/student-documents.js?v=8.0P.167.304');
    const cleanup = module.mountStudentDocuments?.();
    if (typeof cleanup === 'function') registerCleanup(cleanup, 'student-documents');
  } finally {
    window.__SBI_APP_SHELL_MOUNTING_STUDENT_DOCUMENTS = false;
  }

  return { viewKey: 'student:documents' };
}

// SBI 8.0P.167.287 — Livret d'apprentissage numérique (admin / élève / teacher).
async function mountAdminBooklets({ url }) {
  cleanupCourseEditorV2Artifacts();
  maybeCacheAdminIndexMain('leave-for-admin-booklets');
  ...
  applyBodyRouteClassesFromDocument(doc, ['sbi-admin-surface']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Livret d\'apprentissage - SBI Admin');
  setLeftNavActive('nav-formations');
  updateUrlContext(url);

  window.__SBI_APP_SHELL_MOUNTING_ADMIN_BOOKLETS = true;
  try {
    const module = await import('/admin/js/apprenticeship-booklets.js?v=8.0P.167.304');
    const cleanup = module.mountAdminBooklets?.();
    if (typeof cleanup === 'function') registerCleanup(cleanup, 'admin-booklets');
  } finally {
    window.__SBI_APP_SHELL_MOUNTING_ADMIN_BOOKLETS = false;
  }

  return { viewKey: 'admin:booklets' };
}

async function mountStudentBooklet({ url }) {
  cleanupCourseEditorV2Artifacts();
  ...
  applyBodyRouteClassesFromDocument(doc, ['no-right-panel']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Mon livret - SBI Student');
  setLeftNavActive('/student/livret.html');
  updateUrlContext(url);

  window.__SBI_APP_SHELL_MOUNTING_STUDENT_BOOKLET = true;
  try {
    const module = await import('/student/js/student-livret.js?v=8.0P.167.304');
    const cleanup = module.mountStudentBooklet?.();
    if (typeof cleanup === 'function') registerCleanup(cleanup, 'student-livret');
  } finally {
    window.__SBI_APP_SHELL_MOUNTING_STUDENT_BOOKLET = false;
  }

  return { viewKey: 'student:booklet' };
}

async function mountTeacherBooklets({ url }) {
  cleanupCourseEditorV2Artifacts();
  ...
  applyBodyRouteClassesFromDocument(doc, ['sbi-teacher-surface', 'no-right-panel']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Livrets - SBI Prof');
  setLeftNavActive('/teacher/livrets.html');
  updateUrlContext(url);

  window.__SBI_APP_SHELL_MOUNTING_TEACHER_BOOKLETS = true;
  try {
    const module = await import('/teacher/js/teacher-livrets.js?v=8.0P.167.304');
    const cleanup = module.mountTeacherBooklets?.();
    if (typeof cleanup === 'function') registerCleanup(cleanup, 'teacher-booklets');
  } finally {
    window.__SBI_APP_SHELL_MOUNTING_TEACHER_BOOKLETS = false;
  }

  return { viewKey: 'teacher:booklets' };
}
...
async function mountLiveSchedulerV2Route({ url, role = 'teacher' }) {
  cleanupCourseEditorV2Artifacts();
  if (role === 'admin') maybeCacheAdminIndexMain('leave-for-admin-lives-v2');
  ...
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, role === 'admin' ? 'Lives V2 secours - SBI Console' : 'Lives - SBI Teacher');
  setLeftNavActive(role === 'admin' ? 'nav-cursus' : '/teacher/lives.html');
  updateUrlContext(url);

  const module = await import('/js/live/live-scheduler-v2-page.js?v=8.0P.167.357');
  const cleanup = module.mountLiveSchedulerV2Page?.(role);
  if (typeof cleanup === 'function') registerCleanup(cleanup, role === 'admin' ? 'admin-lives-v2' : 'teacher-lives-v2');

  return { viewKey: role === 'admin' ? 'admin:lives-v2' : 'teacher:lives-v2' };
}

async function mountAdminLivesV2({ url }) {
  return mountLiveSchedulerV2Route({ url, role: 'admin' });
}

async function mountTeacherLivesV2({ url }) {
  return mountLiveSchedulerV2Route({ url, role: 'teacher' });
}
...
async function mountStudentLives({ url }) {
  ...
  try {
    const module = await import('/student/js/student-lives.js?v=8.0P.167.247');
...
async function mountStudentLiveReplay({ url }) {
  ...
  try {
    const module = await import('/student/js/live-replay.js?v=8.0P.167.247');
...
async function mountTutorDashboard({ url }) {
  ...
  try {
    const module = await import('/tutor/js/tutor-dashboard.js?v=8.0P.167.304');
...
async function mountTutorBooklet({ url }) {
  ...
  try {
    const module = await import('/tutor/js/tutor-livret.js?v=8.0P.167.304');
...
async function mountTutorDocuments({ url }) {
  cleanupCourseEditorV2Artifacts();
  const doc = await fetchAdminDocument(url);
  await ensureDocumentStyles(doc, url.href);
  applyBodyRouteClassesFromDocument(doc, ['sbi-tutor-surface', 'no-right-panel']);
  replaceMainFromDocument(doc);
  updateAdminChromeFromDocument(doc, 'Documents de formation - SBI Tuteur');
  setLeftNavActive('/tutor/documents.html');
  updateUrlContext(url);
  window.__SBI_APP_SHELL_MOUNTING_TUTOR_DOCUMENTS = true;
  try {
    const module = await import('/tutor/js/tutor-documents.js?v=8.0P.167.304');
    const cleanup = module.mountTutorDocuments?.();
    if (typeof cleanup === 'function') registerCleanup(cleanup, 'tutor-documents');
  } finally {
    window.__SBI_APP_SHELL_MOUNTING_TUTOR_DOCUMENTS = false;
  }
  return { viewKey: 'tutor:documents' };
}
...
async function mountCourseEditorV2Page({ url, role = 'teacher' }) {
  ...
  try {
    const module = await import('/js/course-editor-v2/course-editor-v2.js?v=8.0P.167.358');
...
export function createRouteRegistry() {
  ...
  routes.push({ id: 'tutor-documents', canHandle(url) { return isTutorDocuments(url) && isTutorShellContext(); }, mount: mountTutorDocuments });
  ...
  routes.push({ id: 'teacher-documents', canHandle(url) { return isTeacherDocuments(url) && isTeacherShellContext(); }, mount: mountTeacherDocuments });
  ...
  routes.push({ id: 'student-documents', canHandle(url) { return isStudentDocuments(url) && isStudentShellContext(); }, mount: mountStudentDocuments });
  routes.push({ id: 'student-booklet', canHandle(url) { return isStudentBooklet(url) && isStudentShellContext(); }, mount: mountStudentBooklet });
